  ...
  var fecha = new Date();
    ...
    if (req.session.user ) {
       if((sumMinutes(fecha) <= req.session.user.last_activity+30) || (req.session.user.noclose == 'enabled')){
        ...
        .then(function(u){
          req.session.user = {id:u.id, login:u.login, name:u.name, role: u.role, last_activity:sumMinutes(new Date()), token: u.login+"by"+u.hashed_password};
          next();
        })
...
exports.create = function(req, res, next) {
    ...
    uc.autenticar(req._remoteAddress, login, password, function(error, user) {
        ...
        .then(function(user) {
          ...
          .then(function(){
          req.session.user = {id:user.id, login:user.login, name:user.name, role: user.role, last_activity:sumMinutes(new Date()), token: user.login+"by"+user.hashed_password};
           models.Grab
